package com.operational.platform.spider.util.cookie;

import java.text.SimpleDateFormat;
import java.util.*;

/**
 * An HttpCookieEx object represents an HTTP cookie, which carries state
 ...
 * </blockquote>
 * <p>
 * <p> HttpCookieEx class can accept all these 3 forms of syntax.
 *
 * @author Edward Wang
 * @since 1.6
 */
public class HttpCookieEx {
    // ---------------- Fields --------------
    ...
    static {
        assignors.put("comment", new CookieAttributeAssignor() {
            public void assign(HttpCookieEx cookie,
                               String attrName,
                               String attrValue) {
    ...
    public HttpCookieEx(String name, String value) {
    ...
    private HttpCookieEx(String name, String value, String header) {
        name = name.trim();
     ...
     */
    public static List<HttpCookieEx> parse(String header) {
    ...
    // class.
    private static List<HttpCookieEx> parse(String header, boolean retainHeader) {
        ...
        List<HttpCookieEx> cookies = new ArrayList<>();
        ...
        if (version == 0) {
            ...
            HttpCookieEx cookie = parseInternal(header, retainHeader);
            cookie.setVersion(0);
            cookies.add(cookie);
        } else {
            // rfc2965/2109 cookie
            ...
            for (String cookieStr : cookieStrings) {
                HttpCookieEx cookie = parseInternal(cookieStr, retainHeader);
                cookie.setVersion(1);
                cookies.add(cookie);
            }
        }
     ...
     *
     * @return  an HttpCookieEx being extracted
     *
     * @throws  IllegalArgumentException
     *          if header string violates the cookie specification
     */
    private static HttpCookieEx parseInternal(String header,
                                              boolean retainHeader) {
        HttpCookieEx cookie = null;
    ...
    private static void assignAttribute(HttpCookieEx cookie,
                                        String attrName,
                                        String attrValue) {
    ...
    /**
     * @param toDiscard the toDiscard to set
     */
    ...
    public boolean equals(Object obj) {
        if (obj == this)
            return true;
        if (!(obj instanceof HttpCookieEx))
            return false;
        HttpCookieEx other = (HttpCookieEx) obj;
